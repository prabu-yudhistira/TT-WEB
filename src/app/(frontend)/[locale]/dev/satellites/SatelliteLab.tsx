...
const GROUPS: { title: string; rows: Row[] }[] = [
  ...
  },
  {
    title: 'satellites',
    rows: [
      { key: 'SAT_SIZE', label: 'Size', min: 1, max: 20, step: 0.25 },
...
// Three genuinely different readings of "satellites orbiting the logo", so the
// geometry question gets answered by looking rather than by describing.
const PRESETS: { name: string; note: string; patch: Partial<SatelliteConfig> }[] = [
  {
    name: 'Disk',
    note: 'the reference, translated — dense, near edge-on, long streaks',
    patch: {
      DUST_COUNT: 900,
      DUST_SIZE: 2.2,
      DUST_ALPHA: 0.3,
  ...
  {
    name: 'Halo',
    note: 'face-on ring, calmer and wider — reads as orbit, not accretion',
    patch: {
      DUST_COUNT: 420,
      DUST_SIZE: 2.8,
      DUST_ALPHA: 0.26,
  ...
  {
    name: 'Swarm',
    note: 'many inclinations — satellites around a planet rather than a disk',
    patch: {
      DUST_COUNT: 560,
      DUST_SIZE: 2.4,
      DUST_ALPHA: 0.24,
